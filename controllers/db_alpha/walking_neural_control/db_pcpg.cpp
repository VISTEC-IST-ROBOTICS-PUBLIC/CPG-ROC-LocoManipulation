...
PCPGtest::PCPGtest():set(2),countup(2),countdown(2)
{
	/*******************************************************************************
...
void PCPGtest::updateOutputs(){
			...
			if (pcpg_step.at(0)>= threshold) ////////////////////Intuitively select
			{
				set.at(0) = 1.0;
			}
			if (pcpg_step.at(0)< threshold) ////////////////////Intuitively select
			...
			if (pcpg_step.at(1)>= threshold) ////////////////////Intuitively select
			{
				set.at(1) = 1.0;
			}
			if (pcpg_step.at(1)< threshold) ////////////////////Intuitively select
			...
			if (countup.at(0) == 0.0 && diffset.at(0) == -2.0 && set.at(0) == -1.0){
				...
				// deltaxup.at(0) = 1; // comment if using original program for quick swing
				if (deltaxup.at(0) > uplimit) {
					deltaxup.at(0) = uplimit;
				}
			}
	//
			if (countdown.at(0) == 0.0 && diffset.at(0) == 2.0 && set.at(0) == 1.0){
				...
				// deltaxdown.at(0) = 52;
				if (deltaxdown.at(0) > downlimit) {
					deltaxdown.at(0) = downlimit;
				}
				lastoutput_stance[0] = ydown.at(0);
			}
	//
	//
			if (countup.at(1) == 0.0 && diffset.at(1) == -2.0 && set.at(1) == -1.0){
				deltaxup.at(1) = countupold.at(1);
				// deltaxup.at(1) = 3; // comment if using original program for quick swing
				if (deltaxup.at(1) > uplimit) {
					deltaxup.at(1) = uplimit;
				}				
			}
	//
			if (countdown.at(1) == 0.0 && diffset.at(1) == 2.0 && set.at(1) == 1.0){
				...
				// deltaxdown.at(1) = 52;
				if (deltaxdown.at(1) > downlimit) {
					deltaxdown.at(1) = downlimit;
				}
				lastoutput_stance[1] = ydown.at(1);
			}
			...
			yup.at(0) = ((2./deltaxup.at(0))*xup.at(0))-lastoutput_stance[0]; // from simulation
			...
			yup.at(1) = ((2./deltaxup.at(1))*xup.at(1))-lastoutput_stance[1]; // from simulation
			////////////Scaling  Slope Down calculation//////
...
void PCPGtest::setThreshold(double _threshold){
